...
function shapesOverlap(a, b, gap = MIN_GAP) {
    ...
    for (const decision of decisionShapes) {
        ...
        if (outgoingConnections.length >= 2 && !hasYes && !hasNo) {
            ...
            if (outgoingConnections[0])
                outgoingConnections[0].label = 'Yes';
            if (outgoingConnections[1])
                outgoingConnections[1].label = 'No';
        }
...
function sendToPlugin(sessionCode, command) {
    ...
    if (typeof command === 'object' && command !== null) {
        ...
        if (cmd.type === 'create-diagram' && cmd.data) {
            ...
            const fixedDiagram = validateAndFixDiagram(cmd.data);
...
const createServer = () => {
    ...
    server.registerTool('create_flowchart', {
    ...
    server.registerTool('check_figjam_status', {
    ...
    server.registerTool('get_diagram_rules', {
        ...
        try {
            ...
            if (!sessionId && req.method?.toUpperCase() === 'POST') {
                ...
                transport.onclose = () => {
                    ...
                    if (currentId && transports[currentId])
                        delete transports[currentId];
                    if (currentId && servers[currentId])
                        delete servers[currentId];
                };
